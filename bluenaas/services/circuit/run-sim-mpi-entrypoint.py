...
def run_circuit_simulation(simulation_config, cell_ids, pc, rank, logger):
    ...
    logger.info(
        f"Rank {rank}: Simulation run completed in {sim_run_end_time - sim_run_start_time:.2f} seconds."
    )
    ...
    if rank == 0:
        ...
        if not cell_ids:
            ...
            logger.info(
                f"Rank 0: Plotting took {plotting_end_time - plotting_start_time:.2f} seconds (no cells)."
            )
            return

        fig, axs_list = plt.subplots(
            len(cell_ids), 1, figsize=(10, 6 * len(cell_ids)), squeeze=False
        )
        ...
        if time_ms is None:
            ...
            plotting_end_time = time.perf_counter()
            logger.info(
                f"Rank 0: Plotting took {plotting_end_time - plotting_start_time:.2f} seconds (time trace error)."
            )
            return

        for i, cid_tuple in enumerate(cell_ids):
            voltage_mv = sim.get_voltage_trace(
                cid_tuple
            )  # Get the voltage trace for the specific cell ID
            if voltage_mv is not None:  # Plot if voltage data is available
                ...
                dat_filename = (
                    f"{results_dir}/voltage_{cid_tuple[0]}_{cid_tuple[1]}.dat"
                )
                try:
                    np.savetxt(
                        dat_filename,
                        np.column_stack((time_ms, voltage_mv)),
                        header="Time (ms) Voltage (mV)",
                        fmt="%s",
                    )
                    logger.info(
                        f"Rank 0: Saved .dat file for {cid_tuple} to {dat_filename}"
                    )
                except Exception as e:
                    logger.error(
                        f"Rank 0: Failed to save .dat file for {cid_tuple} to {dat_filename}: {e}"
                    )
            else:
                logger.warning(
                    f"Rank 0: No voltage trace found for {cid_tuple} for plotting or .dat saving."
                )
            ...
            axs_list[i].set_title(
                f"Voltage Trace for Cell {cid_tuple[0]} GID {cid_tuple[1]}"
            )
            axs_list[i].grid()
        plt.tight_layout()
        ...
        plotting_end_time = time.perf_counter()
        logger.info(
            f"Rank 0: Plotting took {plotting_end_time - plotting_start_time:.2f} seconds."
        )
        ...
        if not cell_ids:  # This check is somewhat redundant due to earlier return, but good for clarity
            logger.warning(
                "Rank 0: No cell data to save to NWB (should have been caught earlier)."
            )
            nwb_saving_end_time = time.perf_counter()
            logger.info(
                f"Rank 0: NWB saving took {nwb_saving_end_time - nwb_saving_start_time:.2f} seconds (no cells)."
            )
            return
        ...
        nwbfile = NWBFile(
            ...
            session_id=f"sim_{circuit_folder_name}_{cell_ids[0][0]}_{cell_ids[0][1]}"
            if cell_ids
            else f"sim_{circuit_folder_name}_no_cells",
        )

        device = nwbfile.create_device(
            name="SimulatedIntracellularElectrode",
            description="Virtual electrode simulation.",
        )
        ...
        if time_ms is not None:
            ...
            for i, cid_tuple in enumerate(cell_ids):
                voltage_mv_nwb = sim.get_voltage_trace(
                    cid_tuple
                ...
                if voltage_mv_nwb is not None:
                    ...
                    logger.warning(
                        f"Rank 0: No voltage trace found for {cid_tuple} for NWB saving, skipping NWB entry."
                    )
        else:
        ...
        nwb_saving_end_time = time.perf_counter()
        logger.info(
            f"Rank 0: NWB saving took {nwb_saving_end_time - nwb_saving_start_time:.2f} seconds."
        )
...
def main():
    ...
    parser.add_argument(
        "--config", type=Path, help="Path to the simulation_config JSON file"
    )
    parser.add_argument(
        "--node",
        type=str,
        default="NodeA",
        help="Node population name (default: NodeA)",
    )
    parser.add_argument(
        "--start_gid", type=int, default=0, help="Cell GID (default: 0)"
    )
    parser.add_argument(
        "--num_cells",
        type=int,
        default=1,
        help="Number of cells to simulate (default: 1)",
    )
    ...
    #     log_file_path = (
    #         logs_dir
    #         / f"{circuit_folder_name_for_log}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    #     )
    ...
    cell_ids = [(args.node, args.start_gid + i) for i in range(args.num_cells)]
    ...
    if rank == 0:
        logger.info(
            f"Total script execution time: {overall_end_time - overall_start_time:.2f} seconds."
        )
