...
class JobStream(Stream):
    ...
    def __init__(self, stream_key: str, ctx: dict[str, Any] | None = None):
    ...
    def send_status(self, job_status: JobStatus, extra: str | None = None):
        status_message = JobStatusMessage(status=job_status, extra=extra, ctx=self.ctx)
        self.send(status_message.model_dump_json())

    def send_data(self, data: Any, data_type: str | None = None):
        data_message = JobDataMessage(data=data, data_type=data_type, ctx=self.ctx)
